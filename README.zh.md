...
| 功能 | 描述 |
|------|------|
| 🖱️ **拖动排序** | 常规模式下直接拖动图标排序，无需进入管理面板。 |
| ✨ **实时拖动动画** | 被拖动项放大跟随光标，其他项平滑滑开让位。 |
| 📜 **自动滚动** | 靠近顶部或底部边缘时列表自动滚动。 |
| 🖥️ **多显示器** | 鼠标跨显示器时边栏平滑跟随。 |
| 🌐 **双语支持** | 完整的英语 / 简体中文，全窗口实时同步。 |
| 🔙 **智能返回按钮** | 历史记录只有一页时自动隐藏返回按钮。 |
| 🍪 **独立会话** | 每个应用独立 WebView，多账号无冲突。 |
...
├── src-tauri/                  # Rust 后端
│   ├── src/
│   │   ├── lib.rs              # 入口：插件注册、托盘、命令分发
│   │   ├── state.rs            # 全局原子变量
│   │   ├── monitor.rs          # 鼠标与屏幕工具
│   │   ├── inject.rs           # JS 注入常量
│   │   ├── animation.rs        # 自动隐藏与滑动线程
│   │   ├── window.rs           # 窗口管理命令
│   │   └── commands.rs         # 杂项命令 + 单例锁
│   ├── inject/
│   │   └── navbar.js           # 悬浮导航栏脚本
│   ├── Cargo.toml
│   ├── tauri.conf.json
...
> **v0.2.0 更新**：排序不再需要进管理面板，常规模式下直接拖动图标即可。管理模式仍保留删除和设置功能。
...
- 拖动排序时图标偶尔会轻微漂移，和 WebView2 的 transform 渲染精度有关，不影响最终落位。
