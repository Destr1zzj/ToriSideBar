...
use winapi::um::winuser::{RegisterHotKey, UnregisterHotKey, MOD_ALT, MOD_CONTROL, MOD_SHIFT, MOD_WIN};
...
// ------------------------------------------------------------------
// Test if a global shortcut is available (not taken by another app)
// ------------------------------------------------------------------

fn parse_shortcut_to_vk(key: &str) -> Option<UINT> {
    match key {
        "Space" => Some(0x20),
        ...
        "CapsLock" => Some(0x14),
        "VolumeUp" => Some(0xAE),
        "VolumeDown" => Some(0xAD),
        "VolumeMute" => Some(0xAD),
        "MediaPlayPause" => Some(0xB3),
        "MediaNextTrack" => Some(0xB0),
        "MediaPrevTrack" => Some(0xB1),
        "MediaStop" => Some(0xB2),
        "," => Some(0xBC),
        "." => Some(0xBE),
        "/" => Some(0xBF),
...
#[tauri::command]
pub fn test_global_shortcut(shortcut: String) -> Result<bool, String> {
    ...
    for part in &parts[..parts.len() - 1] {
        match *part {
            ...
            "Command" | "Super" | "Win" => mods |= MOD_WIN,
            _ => return Err(format!("Unknown modifier: {}", part)),
        }
    }

    let vk = parse_shortcut_to_vk(key_part)
        .ok_or_else(|| format!("Unknown key: {}", key_part))?;

    unsafe {
        // Use a temporary dummy window handle (NULL works for global hotkeys in some contexts,
        // but RegisterHotKey requires a valid HWND on Windows. We use HWND 0 which will fail
        // for window-specific hotkeys, but for our test purpose we try anyway.)
        // Actually, RegisterHotKey with NULL hwnd registers a thread-local hotkey.
        let hwnd = std::ptr::null_mut();
        let id = 0x7FFF; // arbitrary test id

        let result = RegisterHotKey(hwnd, id, mods, vk);
        if result == 0 {
            ...
            return Err(format!("RegisterHotKey failed with error {}", err));
        }
        // Immediately unregister to clean up
