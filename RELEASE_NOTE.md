...
| Child windows for same-domain links | ✅ |
| DevTools (F12) in app windows | ✅ |

### What's New in v0.1.6

**Fixed**
- Child window now correctly opens for same-domain `target="_blank"` links and `window.open()` calls.
- Fixed Tauri v2 IPC parameter naming mismatch (`parent_label` → `parentLabel`).
- Relative URLs are now resolved to absolute URLs before being passed to the backend.
- Domain matching now uses root-domain comparison instead of exact hostname match (e.g. `www.bilibili.com` and `space.bilibili.com` are treated as same domain).
- Child window positioning now uses the parent window's monitor instead of the mouse monitor, fixing invisible windows on multi-monitor setups.

**Improved**
- Added `devtools` support: press **F12** in any app window to open Chrome DevTools.
- Added backend `println!` logs for easier debugging of child window creation.
...
- **Child windows**: When browsing inside an app, clicking a same-domain `target="_blank"` link opens a child window to the left of the parent.
...
**修复**
- 子窗口现在可以正确打开：修复了同域 `target="_blank"` 链接和 `window.open()` 无法打开子窗口的问题。
- 修复 Tauri v2 IPC 参数命名不匹配（`parent_label` → `parentLabel`）。
- 相对路径 URL 现在会在传给后端前解析为绝对路径。
- 同域判断从精确 hostname 匹配改为根域名匹配（例如 `www.bilibili.com` 和 `space.bilibili.com` 被视为同域）。
- 子窗口位置计算从「鼠标所在显示器」改为「父窗口所在显示器」，修复多显示器环境下子窗口飞到看不见位置的问题。

**改进**
- 开启 `devtools` 支持：在任意应用窗口按 **F12** 可调出 Chrome DevTools。
- 为子窗口创建流程添加后端 `println!` 日志，方便调试。
...
- **子窗口**：在应用内浏览时，点击同域的 `target="_blank"` 链接会在父窗口左侧弹出子窗口。
